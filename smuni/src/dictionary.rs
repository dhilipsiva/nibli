...
//!
//! Arity comes from `smuni_dictionary`'s jbovlaste dictionary (the one PHF map
...
/// Interface to the jbovlaste arity dictionary (delegates to `smuni_dictionary`).
...
impl JbovlasteSchema {
    ...
    pub fn get_arity(word: &str) -> Option<usize> {
        smuni_dictionary::get_arity(word)
    }
    ...
    pub fn get_arity_or_default(word: &str) -> usize {
        smuni_dictionary::get_arity(word).unwrap_or(2)
    }
}
