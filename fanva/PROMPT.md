...
- Both compile to `wasm32-unknown-unknown` (pure Rust). **Dictionary caveat (rev-dependent):** at a PRE-corpus `<PIN>` (anything before 2026-07-17), `nibli-lexicon`'s `build.rs` reads a gitignored `dictionary-en.json` and a plain git dep falls back to a curated ~140-entry table — for full-vocab arity use a **path dep against a local clone** and run its `just fetch-dict`. At current nibli main the dictionary is a COMMITTED Rust corpus (~1,342 entries, no build.rs, no JSON, English-keyed — gismu spellings no longer resolve), so a git dep gets the full table with zero setup, but the API is the corpus one, not the Lojban-era alias map. (Note jbotci `vlacku` also provides authoritative place structure at runtime — lean on it.)
