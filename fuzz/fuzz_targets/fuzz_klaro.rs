...
// Fuzz the Klaro front-end: arbitrary UTF-8 through `klaro::parse_checked`
// (pest parse -> fail-closed resolve -> emit). Any `Ok(AstBuffer)` has passed
// klaro's own name/place checks, so handing smuni a STRUCTURALLY invalid
// buffer (out-of-bounds index, reference cycle — smuni's `validate_ast_buffer`
// rejects both with the stable "corrupt AST buffer" message) is a klaro
...
fuzz_target!(|data: &[u8]| {
    if let Ok(input) = std::str::from_utf8(data) {
        if let Ok(buffer) = klaro::parse_checked(input) {
            if let Err(e) = smuni::compile_from_gerna_ast(buffer) {
                let msg = format!("{e}");
                assert!(
                    !msg.contains("corrupt AST buffer"),
                    "klaro emitted a structurally invalid AstBuffer for {input:?}: {msg}"
                );
            }
        }
    }
