...
- **Persistence** uses postcard (binary) over the same serde derives — `nibli-engine` stores
  ...
  variant.
- **WIT** ([wit/world.wit](wit/world.wit), package `nibli:engine@0.5.0`) declares the same types
  ...
  carry named-field payload records (`exists-witness-rule { var, term }`) mirroring
...
- The WIT `logic-types` interface (`nibli:engine@0.5.0`).
...
(`nibli:engine@0.5.0`) and the persistence layers' fail-closed schema versions (`nibli-store`)
