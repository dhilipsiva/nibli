...
mod pipeline_bind {
    wasmtime::component::bindgen!({ path: "../wit/world.wit", world: "nibli-pipeline" });
}

use pipeline_bind::nibli::engine::compute_backend;
use pipeline_bind::nibli::engine::error_types::NibliError;
use pipeline_bind::nibli::engine::logic_types::LogicalTerm as EngineLogicalTerm;
...
impl Repl {
    ...
    fn instantiate_session(
        ...
        let pipeline = pipeline_bind::NibliPipeline::instantiate(&mut store, component, linker)?;
...
fn main() -> Result<()> {
    ...
    let wasm_path = std::env::var("NIBLI_WASM_PATH")
        .unwrap_or_else(|_| "target/wasm32-wasip2/debug/nibli.wasm".to_string());
