...
- **Bare-universal branch correctly asserts no xorlo witness (documented boundary — verified 2026-06-26, no action).** The existential-import presupposition fires only for DESCRIPTION universals (`ro lo`/`ro le`, smuni-named `_v{n}`, which carry import and always compile to `∀x. R(x) → C(x)` → the implication branch → `register_clause_rule`'s `is_description_universal` guard asserts the witness). The bare-universal branch (`compile_forall_to_rule` `None` arm) is reached only by a restrictor-less prenex `ro da zo'u da broda` — a plain logical ∀ with NO existential import (vacuously true on an empty domain), so asserting no witness is correct (rejecting bare universals would wrongly break a sound, corpus-used construct). Documented in-place + pinned by `test_bare_universal_asserts_no_presupposition_witness` (the bare branch) and `test_prenex_universal_asserts_no_presupposition_witness` (the implication-branch prenex).
