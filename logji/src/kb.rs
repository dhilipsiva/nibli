...
pub(super) fn register_ground_material_conditional(
    ...
    let Ok(node) = get_node(buffer, node_id) else { return Ok(false) };
...
fn contains_count_node(buffer: &LogicBuffer, node_id: u32) -> bool {
    let Ok(node) = get_node(buffer, node_id) else { return false };
    match node {
...
pub(super) fn process_assertion(inner: &mut KnowledgeBaseInner, logic: &LogicBuffer) -> Result<(), String> {
    for &root_id in &logic.roots {
        ...
        if is_forall {
            ...
            let registered = register_ground_material_conditional(logic, root_id, &skolem_subs, inner)?;
