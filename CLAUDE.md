...
| `just ci` | Fast native CI gate (fmt-check, clippy, all native test suites incl. `test-gasnu` + `test-fanva` + `verify-soundness` + `verify-klaro` + `verify-parser` + `verify-dict` + `verify-proofs`). No WASM build. |
