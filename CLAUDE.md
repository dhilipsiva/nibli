...
| `just verify-soundness` | Differential soundness gate (Track A), **two oracles**: **(1) Vampire** (classical FOL) over the Horn/NAF-free fragment — curated cases, a seeded batch of random Horn programs (`NIBLI_VERIFY_RANDOM_COUNT`, default 200; `nibli-verify --random N` for a manual sweep), and the auto-extracted mappable slice of the `gdpr`/`ddi` corpora; **(2) clingo** (ASP/Datalog) over the stratified-NAF + closed-world fragment Vampire can't cover — a curated NAF corpus + random stratified-NAF programs (`NIBLI_VERIFY_NAF_RANDOM_COUNT`, default 100; `nibli-verify --asp [--random N]`), where nibli's closed-world verdict must equal clingo's unique stable/perfect model. Needs `vampire` + `clingo` (Nix shell; each side skips if its solver is absent). Part of `ci`. |
