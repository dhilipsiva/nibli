...
| `just ci` | Fast native CI gate (fmt-check, clippy, all native test suites incl. `test-gasnu` + `test-ui` + `test-fanva` + `verify-soundness` + `verify-nibli-kr-dict` + `verify-nibli-kr-seam` + `verify-dict` + `verify-proofs`). No WASM build. |
...
| `just fuzz-nibli-kr [SECS]` | Fuzz the Klaro front-end (parse → resolve → emit): any accepted input must compile through smuni WITHOUT a "corrupt AST buffer" rejection — a structurally invalid emitted buffer is a klaro bug, surfaced as an oracle panic (plus the usual crash/leak detection) |
...
| `nibli-engine` | — | Native in-process embedding of the pipeline (used by tests + the store layer; no Wasmtime). `compile_text` is the sole text→AST seam (`nibli_kr::parse_checked` → smuni → compute-marking); buffer replay is text-free | `lib.rs` |
