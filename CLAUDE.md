...
| `just test-fanva` | Run nibli-fanva native tests (agentic loop + history trim, local gates, LLM request/response shapes, MCP wire/types, tool loop). Part of `ci`. |
...
| `nibli-types` | — | Shared flat WIT-compatible types used by every stage: `AstBuffer`, `LogicBuffer`/`LogicNode`, `NibliError`, and the single shared arithmetic evaluator (tolerant equality) | `ast.rs`, `logic.rs`, `error.rs`, `arithmetic.rs` |
