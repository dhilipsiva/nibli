...
| `klaro` | clear (working name) | The Klaro surface-syntax front-end (IN PROGRESS — `KLARO_TODO.md` program; spec `SURFACE_SYNTAX.md`): parses the predicate-call language (`goes(me, to: some market).`) into the same `AstBuffer` gerna produces, reusing smuni/logji unchanged. Landed so far: the logos lexer — maximal-munch tokens, exact-match keyword reservation pinned both-directions against klaro-dictionary's reserved list, positioned fail-closed errors (non-ASCII syntax, unknown escapes, non-finite numbers). No public compile entry point until the parser bullets land | `src/token.rs`, `src/lexer.rs` |
