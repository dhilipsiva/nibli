...
| `nibli-fanva` | translate | Agentic English→Lojban translator engine consumed by nibli-ui: a multi-turn LLM client (5 providers) with a `Chat` seam + wasm `HttpChat` transport, the local validation gates (`gerna::parse_checked` + `smuni::compile_from_gerna_ast`; the camxes "official" gate + jbotci MCP tool-use are later phases), and the `translate_agentic` loop (line-by-line KB validation, attempt cap, oscillation guard). Pure logic is native-tested; the browser transport is wasm-only | `agent.rs`, `gates.rs`, `llm/` |
