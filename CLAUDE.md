...
| `klaro` | clear (working name) | The Klaro surface-syntax front-end (IN PROGRESS — `KLARO_TODO.md` program; spec `SURFACE_SYNTAX.md`): parses the predicate-call language (`goes(me, to: some market).`) into the same `AstBuffer` gerna produces, reusing smuni/logji unchanged. Landed so far: the logos lexer (maximal-munch tokens, exact-match keyword reservation pinned both-directions against klaro-dictionary's reserved list, positioned fail-closed errors) + the parser core (recursive descent → owned tree AST: terms, determiners, positional+named args, operator chain, tense/deontic prefixes, binary `=`, per-statement error recovery; the §6 errata enforced as grammar — `~past P`/`~~P`/prefix-or-`~`-over-compound rejected with targeted errors). No public compile entry point until the emitter bullet lands | `src/token.rs`, `src/lexer.rs`, `src/ast.rs`, `src/parser.rs` |
