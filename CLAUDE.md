...
| `just verify-soundness` | Differential soundness gate (Track A): nibli's verdict must agree with Vampire over the Horn/NAF-free corpus (needs `vampire`, provided by the Nix shell; skips if absent). Part of `ci`. |
