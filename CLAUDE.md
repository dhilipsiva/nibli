...
| `just ci` | Fast native CI gate (fmt-check, clippy, all native test suites incl. `test-gasnu` + `test-fanva` + `verify-soundness` + `verify-klaro` + `verify-klaro-dict` + `verify-parser` + `verify-dict` + `verify-proofs`). No WASM build. |
...
| `just ci-wasm` | WASM behavioral gate: build the lasna component + run the gasnu smokes (fuel exhaustion, post-trap recovery, go'i variants, persist-replay, multi-`.i` split + buffer replay, NAF note, `:debug`, quiet mode, …) plus the three-way determinism legs: `smoke-gasnu-determinism` (the pinned `determinism-corpus.lojban` through the Wasmtime component) and `verify-wasm-node` (the same corpus through the wasm32-unknown-unknown build under node/V8 via wasm-pack; skips cleanly if wasm-pack is absent). The native leg (`determinism_corpus_native`) rides `verify-soundness`. Also runs `test-fanva-wasm` (nibli-fanva's camxes-gate marshalling tests under node). |
...
| `nibli-ui` | — | Standalone Dioxus web UI (browser, port 8080) — gerna/smuni/logji compiled in, reasons fully in-browser; optional client-side BYO-key LLM Translate (Source→Lojban), key held in-tab-memory only, no server. The Source→Lojban button runs the agentic self-correcting translator (`nibli-fanva`): translate → validate (gerna+smuni+camxes) → fresh-context semantic verification of the back-translation → feed any error back → retry, with a self-correction trace. The LLM client is single-sourced on `nibli_fanva::llm`; nibli-ui holds a thin `Settings { llm, proxy_url, max_attempts }` wrapper. The Back-translation tab shows the local `nibli_render` gloss and, when a jbotci proxy is configured, an optional **deep-meaning view** — jbotci's `tersmu` semantic graph (via `nibli_fanva::mcp::McpClient::tersmu`) rendered verbatim | `main.rs`, `examples.rs` |
| `nibli-fanva` | translate | Agentic English→Lojban translator engine consumed by nibli-ui: a multi-turn LLM client (5 providers) with a `Chat` seam + wasm `HttpChat` transport, the three local validation gates (`gerna::parse_checked` + `smuni::compile_from_gerna_ast` + the "official" **camxes** gate — vendored ilmentufa `camxes.js` called via synchronous js-sys through a `window.camxes_validate` shim nibli-ui serves, same pin as `verify-parser`'s `NIBLI_CAMXES_DIR`), an MCP client (`mcp/`) for jbotci dictionary/grammar tool-use during translation — reached via an optional app-owned proxy (`fanva-proxy/`, a workspace-excluded Cloudflare Worker; degrades to the local gates when no proxy) — the `translate_agentic` loop (line-by-line KB validation, attempt cap, oscillation guard, `degraded` flag), and the **semantic verification turn** (`verify.rs`, int19h feedback): after the gates pass, a FRESH-context judge call reads the overlay-free `nibli_render` back-translation of each KB line and a MISMATCH retries through the same loop as `GateError::Verification` — best-effort advisory (judge outages/malformed verdicts fail open; the deterministic gates stay the hard guarantee). Pure logic is native-tested (incl. the Genesis 1:1–8 garbage-that-parses fixture); the browser transport + camxes gate are wasm-only | `agent.rs`, `gates.rs`, `llm/`, `mcp/`, `tools.rs` |
