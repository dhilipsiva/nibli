...
flipped from gismu to English, so **proof traces contain no Lojban**: content, arithmetic/
comparison, `equals` (the du identity), `something` (zo'e), `$var` names, and the abstraction
type names are all English. The residual user-facing Lojban is narrow — the argument-pronoun
cmavo (`mi`/`do`/`ma`/`ke'a`/`ce'u`/`ko'a`) the emitter still produces, and the L4 alias-echo lint
(`dog ↦ gerku`) — recorded for a follow-up. The remaining ceiling is **adoption** — chiefly a
reproducible non-expert authoring study (round-trip fidelity + silent-mistranslation rate), which
belongs to the book/UX track.
